# Minimalist Grid

Clean, contemporary theme with geometric design aligned to a consistent baseline grid. Light-weight sans-serif with ample letter spacing.

![Minimalist Grid theme preview](https://raw.githubusercontent.com/jsonresume/jsonresume.org/master/apps/homepage2/public/img/themes/minimalist-grid.png)

**Tags:** minimalist, grid, geometric, clean, charcoal

## Use it

Preview it live on the JSON Resume registry:

- https://registry.jsonresume.org/thomasdavis?theme=minimalist-grid

Install and use it with the JSON Resume CLI:

```sh
npm install jsonresume-theme-minimalist-grid
resume export resume.html --theme minimalist-grid
```

## Sections

Renders all standard JSON Resume sections: work, education, skills, projects, volunteer, awards, publications, languages, interests, references.

## Development

This theme lives in the [jsonresume.org](https://github.com/jsonresume/jsonresume.org) monorepo. Build it from the repo root with:

```sh
pnpm --filter jsonresume-theme-minimalist-grid build
```

## License

MIT
