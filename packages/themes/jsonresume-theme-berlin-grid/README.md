# Berlin Grid

Geometric precision with modular grid layout and grotesk sans-serif typography. Feels engineered — angular, structured, and meticulously aligned.

![Berlin Grid theme preview](https://raw.githubusercontent.com/jsonresume/jsonresume.org/master/apps/homepage2/public/img/themes/berlin-grid.png)

**Tags:** grid, geometric, precise, engineer, architect, systematic, minimal

## Use it

Preview it live on the JSON Resume registry:

- https://registry.jsonresume.org/thomasdavis?theme=berlin-grid

Install and use it with the JSON Resume CLI:

```sh
npm install jsonresume-theme-berlin-grid
resume export resume.html --theme berlin-grid
```

## Sections

Renders all standard JSON Resume sections: work, education, skills, projects, volunteer, awards, publications, languages, interests, references.

## Development

This theme lives in the [jsonresume.org](https://github.com/jsonresume/jsonresume.org) monorepo. Build it from the repo root with:

```sh
pnpm --filter jsonresume-theme-berlin-grid build
```

## License

MIT
